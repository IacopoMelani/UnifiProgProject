...
public class KSubsetSum {
  public static boolean kSubsetSum(int[] a, int k, int s) {
    ...
    boolean[] c = { false };
    KSubsetSum.genBinary(c, a, cpa, a.length, s, k);
    return c[0];
  }
  
  public static void genBinary(boolean[] c, int[] a, int[] cpa, int b, int s, int r) {
    boolean ris = false;
    if (((b == 0) && KSubsetSum.kSumArray(cpa, a, s))) {
      ...
      if ((r < b)) {
        a[(b - 1)] = 0;
        KSubsetSum.genBinary(c, a, cpa, (b - 1), s, r);
      }
      if ((r > 0)) {
        a[(b - 1)] = 1;
        KSubsetSum.genBinary(c, a, cpa, (b - 1), s, (r - 1));
      }
    }
  }
